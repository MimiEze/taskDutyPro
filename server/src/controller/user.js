...
export const registerUser = async (req, res, next) => {
  const {email, username, password } = req.body; // receives info from client
  ...
  try {
    ...
    const hashPassword = await bcrypt.hash(password, salt); //encrypt the password
    ...
    res.status(201).json({msg: "User signup successfully", accessToken });
  } catch (error) {
    next(error);
  }
};
