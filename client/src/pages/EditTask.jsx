  ...
  useEffect(() => {
    const getData = async () => {
      ...
      try {
        ...
        console.error(error);
        toast.error(
          error?.response?.data?.error || error?.message || "Än error occured"
        );
    ...
    <div className="container mx-auto p-4">
      ...
      <form className="mt-5" onSubmit={handleSubmit(submitForm)}>
        <fieldset className="fieldset bg-base-200 p-4 rounded-lg">
          <legend className="fieldset-legend text-lg text-grey-500">Task title</legend>
          <input type="text" className="input w-full" placeholder="Enter task title" {...register("title", { required: true })} />
          {errors?.title && (
            <p className="text-xs text-red-600">Title is required</p>
          )}
        </fieldset>
        <fieldset className="fieldset bg-base-200 p-4 rounded-lg">
          <legend className="fieldset-legend text-lg text-grey-500">
            Description
          </legend>
          <textarea className="textarea textarea-md w-full"
        ...
        <fieldset className="fieldset bg-base-200 p-4 rounded-lg">
          ...
          <select className="select w-full" defaultValue="" {...register("tag", { required: true })} >
            <option disabled={true} value="">Select a tag</option>
        ...
        <button className="mt-6 btn btn-lg w-full bg-[#974FD0] text-white" disabled={isSubmitting}> {isSubmitting ? <span className="loading loading-spinner"></span> : "Done"}
        </button>
      </form>
    </div>
  )
